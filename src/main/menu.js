...
function setAllowNav(flag) {
  ...
  if (process.platform === 'darwin') {
    ...
    getMenuItem('Share local Folder...').enabled = flag
    getMenuItem('Share local File...').enabled = flag
  }
}
...
function getMenuTemplate() {
  ...
  const template = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'Send VGo coins',
          click: () => windows.main.dispatch('sendvgoCoin')
        },
        {
          label: 'Servers',
          click: () => windows.main.dispatch('servers')
        }
      ]
    },
    {
      ...
      submenu: [
        {
          ...
          click: () => {
            ...
            shell.openExternal(config.GITHUB_URL_WEB)
          }
        },
        {
          label: 'Android',
          click: () => {
            const shell = require('./shell')
            shell.openExternal(config.GITHUB_URL_ANDROID)
          }
        },
        {
          label: 'iOs',
          click: () => {
            const shell = require('./shell')
            shell.openExternal(config.GITHUB_URL_IOS)
          }
        }
      ]
    },
