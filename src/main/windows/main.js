...
const { getAccountInfo } = require('../../renderer/vgo/manager')
const { parameters } = require('../../renderer/vgo/parameters')
function init(state, options) {
  parameters.syncVGoConfigs((conf) => {
  ...
  win.loadURL(config.WINDOW_LOGIN)
  win.maximize();
...
function showLoginHtml() {
  //ipcRenderer.send('init-webtorrent',peerId,peerToken,peerSignature)
  main.win.loadURL(config.WINDOW_LOGIN)
}

function showMainHtml(pubHex, rsId, account) {
  //ipcRenderer.send('init-webtorrent',peerId,peerToken,peerSignature)
  //dispatch('account-login',pubHex,rsId)
  global.vgoAccount = account;
  main.win.loadURL(config.WINDOW_MAIN)
}

function showTestHtml(pubkHex) {
  //ipcRenderer.send('init-webtorrent',peerId,peerToken,peerSignature)
  //TODO check why this not working
  getAccountInfo(pubkHex, (account) => {
    main.win.loadURL(config.WINDOW_TEST)
  })
}

function showLivePage() {
  main.win.loadURL(config.WINDOW_LIVE)
}
