...
function renderPayButton(torrentSummary) {
  let price = parseFloat(torrentSummary.price);
  if (price == 0)
    return null;
  return (
    <div>
      <span
        onClick={dispatcher('showVgoDialog',torrentSummary.infoHash)}>
        Need to pay VGo coin after 10% preview
  </span>
    </div>)
}
...
function renderLoadingSpinner(state) {
  ...
  let sum = state.getPlayingTorrentSummary()
