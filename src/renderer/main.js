...
function onState(err, _state) {
  ...
  controllers = {
    ...
    account: createGetter(() => {
      const AccountController = require('./controllers/account-controller')
      return new AccountController(state)
    }),
    gege: createGetter(() => {
      const GegevController = require('./gege/gegev-controller')
      return new GegevController(state)
    }),
    wallet: createGetter(() => {
      const WalletController = require('./gege/wallet-controller')
      return new WalletController(state)
    }),
  }

  vgoDB.loadLastAccount(config.ACCOUNT_CACHE, (la) => {
    console.log('loadLastAccount=' + la.rsId + " " + la)
    let aid = 0;
    if (la && la.pk) {
      aid = la.pk
      state.account = { pk: la.pk, balance: la.balance, rsId: la.rsId };
    }

    console.log(state.account);
    getAccountInfo(aid, (account) => {
  ...
  controllers.gege().loadResources(1, 0)
...
const dispatchHandlers = {
  ...
  'openMiningDir' : () => { ipcRenderer.send('openMiningDir')},
  'openTorrentAddress': () => { state.modal = { id: 'open-torrent-address-modal' } },
  'account': (accountId) => controllers.account().showAccount(accountId),
  'sendvgoCoin': () => controllers.wallet().showWallet(),
  ...
  'confirmDeleteTorrent': (infoHash, deleteData) =>
    ...
    controllers.torrentList().showVgoDialog(infoHash),
  'showResourceInfo' : (infoHash) =>controllers.torrentList().showResourceInfo(infoHash),
  ...
  //gege 
  'sendvgo': (tran, cb) => controllers.gege().sendvgo(tran, cb),
  'purchaseResource': (infoHash, cb) => controllers.gege().purchaseGGResource(infoHash, cb),
  'searchFiles': (query) => controllers.gege().loadResources(1, query),
  "reboot": (userSecHex) => controllers.gege().reboot(userSecHex),
  "updateStatus": () => controllers.gege().updateStatus(),
  "blockList": (page) => controllers.gege().showBlockList(page),
  "unconfirmedList": (page) => controllers.gege().showUnconfirmedList(page),
  "servers":()=>controllers.gege().showServerPage(),
  // Playback
  'playFile': (infoHash, index) => controllers.playback().playFile(infoHash, index),
  'buyResource': (infoHash) => controllers.playback().gotoBuyPage(infoHash),
  ...
  'home': () => state.location.go({
    url: 'home'
  }),
  // 'account-login': (pkHex, rsId) => {
  //   console.log('account-login ' + rsId)
  //   state.account = { rsId: rsId, pk: pkHex, balance: 0 }
  // },
  //
  // 'account-status': (status) => {
  //   console.log('account-status ' + status)
  //   var account = JSON.parse(status.account);
  //   state.account = account;
  //   app.setState(state)
  // },
...
function setupIpc() {
  ...
  // 'loadResources' :(page,query,account) => controllers.gege().loadResources(page,query,account),
  ipcRenderer.on('loadResources', (e, ...args) => controllers.gege().loadResources(...args))
