...
const { shareResource } = require('../vgo/manager')
...
module.exports = class TorrentController {
  ...
  torrentInfoHash(torrentKey, infoHash, price) {
    ...
    if (!torrentSummary) {
      ...
      resources.unshift(torrentSummary)
      sound.play('ADD')
    }
  ...
  torrentDone(torrentKey, torrentInfo) {
    ...
    appConfig.read(function (err, saved) {
      ...
      saved.resource.secretHex = userSecHex;
      shareResource(torrentInfo, saved.resource, (ok, err) => {
        if (ok) {
          dispatch('update')
        } else {
          console.log('share resource transaction err.stop torrent' + err);
          //dispatch('deleteTorrent', torrentInfo.infoHash, false);
        }
      })
    })
