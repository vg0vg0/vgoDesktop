...
const { getResourceInfo } = require('../vgo/manager')
...
module.exports = class PlaybackController {
  ...
  playFile(resId, index /* optional */) {
    this.gotoPlayer(resId, index)
  }
  ...
  gotoBuyPage(resId) {
    getResourceInfo(resId, (resource) => {
      state.location.go({
        url: 'purchase',
        setup: (cb) => {
          state.window.title = 'Pay VGo coin'
          state.resId = resId
  ...
  gotoPlayer(infoHash, index /* optional */) {
