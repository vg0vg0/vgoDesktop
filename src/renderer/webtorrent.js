...
const WebTorrent = require('../vgoTorrent')
...
ipc.on('init-peer', (e, pid, userSecHex) => {
  //console.log('init peer in webtorrent pid=%s,userSecHex=%s', pid,userSecHex);
  //client = window.client = new WebTorrent({ peerId: pid })
  // init();
  vgoDB.loadRtcConfig(config.RTCCONFIG_CACHE, (rtcConfig) => {
...
function init() {
  ...
  ipc.on('start-mining', (file, onSeedCallback) => startMining(file, onSeedCallback))
...
function startMining(file, onSeedCallback) {
  console.log('web torrent start mining ' + file)
  client.seed(file, onSeedCallback)
}
...
function createTorrent(torrentKey, options) {
  ...
  torrent.key = torrentKey
  addTorrentEvents(torrent, options.price)
  ipc.send('wt-new-torrent')
}

function addTorrentEvents(torrent, price) {
  torrent.on('warning', (err) =>
    ipc.send('wt-warning', torrent.key, err.message))
  torrent.on('error', (err) =>
    ipc.send('wt-error', torrent.key, err.message))
  torrent.on('infoHash', () =>
    ipc.send('wt-infohash', torrent.key, torrent.infoHash, price))
