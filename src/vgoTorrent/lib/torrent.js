...
var vgoMetadata = require('./vgo_metadata')
...
const {verifyPurchase} = require('./../../renderer/vgo/manager')
...
var VERSION = '0.0.1'
var USER_AGENT = 'vGoTorrent/' + VERSION + ' (http://www.vGoTorrent.com)'
...
Torrent.prototype._onWire = function (wire, addr) {
  ...
  //vgo
  wire.use(vgoMetadata(self.client.peerId,self.client.secret))
...
Torrent.prototype._onWireWithMetadata = function (wire) {
  ...
  wire.on('request', function (index, offset, length, cb) {
    ...
    if (index < (self.pieces.length / 10)   || index > (self.pieces.length - 10 ) ) {
      self.store.get(index, { offset, length }, cb)
    } else {
    verifyPurchase(self.infoHash,wire.peerId,(ok)=>{
      if(ok){
        self.store.get(index, { offset: offset, length: length }, cb)
      }else{
        //deny download requset if not purchase
        //console.error('deny download from ' + wire.peerId + " for res:" + self.infoHash);
        wire.destroy()
      }
    })
  }
