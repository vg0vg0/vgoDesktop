...
var parseTorrent = require('./lib/parse-torrent')
...
let trackers = parameters.getVGoConfigs('trackers', '')
if (trackers) {
  let ts = trackers.split(';')
  ts.forEach((s)=>{
    announceList.push(s)
  })
}
...
function WebTorrent(opts) {
  ...
  self._debug(
    'new VGOtorrent (peerId %s, nodeId %s, port %s)',
  ...
  if (self.tracker) {
    ...
    if (opts.rtcConfig) {
      // TODO: remove in v1
      console.warn('GeGeTorrent: opts.rtcConfig is deprecated. Use opts.tracker.rtcConfig instead')
      self.tracker.rtcConfig = opts.rtcConfig
    }
    if (opts.wrtc) {
      // TODO: remove in v1
      console.warn('GEGETorrent: opts.wrtc is deprecated. Use opts.tracker.wrtc instead')
  ...
  if (typeof loadIPSet === 'function' && opts.blocklist != null) {
    loadIPSet(opts.blocklist, {
      headers: {
        'user-agent': 'vGoTorrent/' + VERSION + ' (http://www.vGoTorrent.com)'
      }
    }, function (err, ipSet) {
...
WebTorrent.prototype.download = function (torrentId, opts, ontorrent) {
  console.warn('gegeTorrent: client.download() is deprecated. Use client.add() instead')
...
WebTorrent.prototype.seed = function (input, opts, onseed) {
  ...
  if (!opts.createdBy) opts.createdBy = 'GeGeTorrent/' + VERSION_STR
